...
* Hello world program
  ...
  * To turn the code block into a source code C file (~.c~)
...
weave Markdown files (~.md~), or WORD (~*.odt~) files, or we
dispense with the weaving altogether (because the Org-mode files
look find on GitHub.
